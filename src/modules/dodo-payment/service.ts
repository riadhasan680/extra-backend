...
export default class DodoPaymentProviderService extends AbstractPaymentProvider<Options> {
  ...
  constructor(container: { logger: Logger }, options: Options) {
    ...
    this.client_ = new DodoPayments({
      ...
      environment:
        options.env === "live" || options.env === "live_mode"
          ? "live_mode"
          : "test_mode",
    });
    this.logger_.info(
      `[Dodo] Initialized with env: ${options.env}, productId: ${options.productId}`,
    );
  }

  async initiatePayment(input: any): Promise<any> {
    this.logger_.info(
      `[Dodo] Initiating payment. Input: ${JSON.stringify(input)}`,
    );

    const productId = this.options_.productId;
    if (!productId || productId.includes("placeholder")) {
      this.logger_.warn(
        `[Dodo] Warning: DODO_PRODUCT_ID is missing or is a placeholder. Please set a valid Product ID in .env.`,
      );
    }
    ...
    if (!customerEmail) {
      this.logger_.warn(
        `[Dodo] Customer email missing in input. Using fallback test email.`,
      );
    ...
    const finalProductId =
      productId && !productId.includes("placeholder")
        ? productId
        : "pdt_0NXUEYPN4KbaluF9re0OO";

    try {
      // Resolve Cart ID
      let cartId =
        input.data?.cart_id || input.resource_id || input.context?.resource_id;

      if (!cartId) {
        this.logger_.error(
          "[Dodo] CRITICAL: Cart ID is missing in initiatePayment input!",
        );
        if (input.payment_session?.data?.cart_id)
          cartId = input.payment_session.data.cart_id;
      }
      ...
      this.logger_.info(
        `[Dodo] Creating session with payload: ${JSON.stringify(sessionPayload, null, 2)}`,
      );

      const session = await this.client_.checkoutSessions.create(
        sessionPayload as any,
      );

      this.logger_.info(
        `[Dodo] Created session: ${session.session_id}, URL: ${session.checkout_url}`,
      );
  ...
  async authorizePayment(input: any): Promise<{
    status: PaymentSessionStatus;
    data: Record<string, unknown>;
  }> {
    this.logger_.info(
      `[Dodo] authorizePayment called. Input: ${JSON.stringify(input)}`,
    );
    ...
    if (!sessionId) {
      this.logger_.error(
        `[Dodo] No Session ID found in authorizePayment input`,
      );
      return {
  ...
  async capturePayment(input: any): Promise<{
    status: PaymentSessionStatus;
    data: Record<string, unknown>;
  }> {
  ...
  async cancelPayment(input: any): Promise<Record<string, unknown>> {
    return input;
  }
  ...
  async refundPayment(input: any): Promise<Record<string, unknown>> {
    return input;
  }

  async retrievePayment(input: any): Promise<Record<string, unknown>> {
    return input;
  }

  async updatePayment(input: any): Promise<Record<string, unknown>> {
